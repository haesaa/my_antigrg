...
- **1-hop 탐색**: `c:\hehe\skills\RAG\catalog.json` 쿼리 -> `small_model`, `review`, `caution` 태그 스캔 후 필요한 RAG 섹션(`D1`, `D3`, `A6` 등) 맵핑 동적 로드
- **정적 참조 캡슐화**: 에이전트는 하드코딩된 `shared/*` 경로 대신 RAG 라이브러리를 통해 최신 지식을 우선 병합한다.
